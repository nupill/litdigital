...
function is_basic_lti_request() {
	...
	// if ($good_message_type and $good_lti_version and isset (
	// $resource_link_id ))
...
class BLTI {
	...
	function __construct($parm = false, $usesession = true, $doredirect = true) {
		...
		if (is_string ( $parm )) {
			...
			$column = 'oauth_consumer_key';
			$sql = 'SELECT * FROM ' . $config ['mysql_table'] . ' WHERE ' . $column . ' = ' . "'" . $oauth_consumer_key . "'";
			
			mysql_connect ( 'localhost', 'dbuser', 'dbpass' ) or die ( 'Error connecting to mysql' );
	...
	function getWelcomeMessage() {
		$nome = $this->info ['nome_usuario'];
		$email = $this->info ['email_usuario'];
	...
	function getConsumerEmail() {
		return $this->info ['email_usuario'];
	}
	...
	function redirect2() {
		$location = "http://www.dlnotes2.ufsc.br/user/login";
		header ( "Location: $location" );
	}
