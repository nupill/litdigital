...
$generos = $controle_documentos->get_generos(DOCUMENTOS_OBJETOS_ARTE_ID);

$idiomasAtuais = $controle_documentos->get_idiomas_new($id, array('id', 'descricao'));
$idiomas = $controle_documentos->get_idiomas();

$categorias = $controle_documentos->get_categorias(DOCUMENTOS_OBJETOS_ARTE_ID);
...
<div id="content">
    <h2>
        ...
        <a href="<?php echo ADMIN_DOCUMENTOS_OBJETOS_ARTE_URI; ?>"><?php echo __('Objetos de Arte');?></a> &raquo;
        <?php echo __('Editar');?>
    </h2>
    <br />
    <form id="update" action="<?php echo ADMIN_DOCUMENTOS_OBJETOS_ARTE_URI; ?>?action=update&id=<?php echo $id; ?>" method="post" class="inline">
        <fieldset>
            <legend><?php echo __('Títulos');?></legend>
            <p><label for="titulo"><?php echo __('Título');?>*</label>
                <input type="text" id="titulo" name="titulo" style="width: 780px" value="<?php echo $documento['titulo']; ?>" /></p>
        </fieldset>
        <fieldset>
