...
class ControleEstados extends DataTables {
...
public function get($id = '', $fields = array(), $start = 0, $limit = 0, $descricao = '') {
        ...
        $query = "SELECT $columns FROM estados";
 ...
 //       }
    ...
    public function getEstado($id, $fields = array()) {
    	$columns = '*';
    	if ($fields) {
    		//Junta os campos/colunas para a consulta SQL
     ...
     */
    public function add($nome, $sigla, $pais, $lat=NULL, $lng=NULL) {
        ...
        //Validação
        $invalid_fields = $this->validate_parameters($nome,$sigla, $pais,$lat,$lng);
        if ($invalid_fields) {
     ...
     * @return string Resultado da inserção no formato JSON
     */
    public function update($id,$nome,$sigla, $pais, $lat=NULL, $lng=NULL) {
        ...
        try {
            $this->DB->update('estados', $fields, $where);
    ...
    public function del($ids, $force=NULL) {
        ...
        if (!is_array($ids) || !$ids) {
        	...
        	try {
        		...
        		$query = sprintf("DELETE FROM estados
     ...
     */
    private function validate_parameters($nome,$sigla,$pais,$lat,$lng) {
        $invalid_fields = array();
        if (!$pais) {
        	$invalid_fields['nome'] = 'Nome do estado deve ser especificado';
        	 
        }
        if (!$nome) {
            $invalid_fields['sigla'] = 'Sigla deve ser especificada';
        }
        if (!$pais) {
        	$invalid_fields['pais'] = 'Nome do estado deve ser especificado';
        }
    	if ($lat or $lng) {
        	if (!is_numeric($lat)) 
