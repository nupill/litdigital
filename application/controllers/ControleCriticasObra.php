...
class ControleCriticasObra extends DataTables {
	...
	public function get($id = '', $fields = array (), $start = 0, $limit = 0) {
		...
		$query = "SELECT $columns FROM CriticaObraLiteraria";
		if ($id) {
	...
	public function get_by_obra($id = '', $fields = array (), $start = 0, $limit = 0) {
		$result = array ();
		$index = 0;
		...
		$query = "SELECT $columns FROM CriticaObraLiteraria";
		if ($id) {
			$query .= sprintf(" WHERE ObraLiteraria_id='%s'", mysqlx_real_escape_string($id));
		}
		elseif (is_numeric($start) && $start >= 0 && is_numeric($limit) && $limit > 0) {
			$query .= sprintf(" LIMIT %u,%u", mysqlx_real_escape_string($start), mysqlx_real_escape_string($limit));
		}
	...
	public function add($titulo, $autor_critica, $obra_id, $obra, $nome_arquivo, $mime, $tamanho_arquivo) {
		...
		//Validation
		$invalid_fields = $this->validate_parameters($titulo, $obra);
		if ($invalid_fields) {
			return json_encode(array (
		...
		$fields['ObraLiteraria_id'] = $obra_id;
		$fields['Usuario_id'] = $_SESSION['id'];
		$fields['Documento_titulo'] = $obra;
		$fields['nome_arquivo'] = $nome_arquivo;
		$fields['mime'] = $mime;
		$fields['tamanho'] = $tamanho_arquivo;

		try {
			$this->DB->insert('CriticaObraLiteraria', $fields, false);
		} catch (Exception $e) {
	...
	public function update($id, $titulo, $autor_critica, $nome_arquivo, $mime, $tamanho_arquivo) {
		...
		try {
			$this->DB->update('CriticaObraLiteraria', $fields, $where, false);
		} catch (Exception $e) {
			Logger :: log($e->getMessage(), __FILE__);
			return json_encode(array (
	...
	public function del($ids) {
		...
		$ids_str = implode("','", $ids);
		$query = sprintf("DELETE FROM CriticaObraLiteraria
	...
	private function validate_parameters($titulo, $obra) {
		...
		if (!$obra) {
			$invalid_fields['obra'] = 'A crítica é atribuida à uma obra';
		}
