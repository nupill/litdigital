<?php
require_once(dirname(__FILE__) . '/../../../../application/config/general.php');
require_once(APPLICATION_PATH . '/controllers/ControleObraLiteraria.php');

$action = isset($_GET['action']) ? $_GET['action'] : '';

switch ($action) {
    
    case 'add':
    case 'update':
        
        //Atributos do Documento:
        $titulo = isset($_POST['titulo']) ? $_POST['titulo'] : '';
        $generos = isset($_POST['generos']) ? $_POST['generos'] : array();
        $categoria = isset($_POST['categoria']) ? $_POST['categoria'] : '';
        $autores = isset($_POST['autores']) ? $_POST['autores'] : array();
        $fontes = isset($_POST['fontes']) ? $_POST['fontes'] : array();
        $id_material = isset($_POST['id_material']) ? $_POST['id_material'] : '';
        $editoras = isset($_POST['editoras']) ? $_POST['editoras'] : array();
                
        $abrangencia = isset($_POST['abrangencia']) ? $_POST['abrangencia'] : '';
        $direitos = isset($_POST['direitos']) ? $_POST['direitos'] : '';
        $acervo_id = isset($_POST['acervo_id']) ? $_POST['acervo_id'] : '';
        $localizacao = isset($_POST['localizacao']) ? $_POST['localizacao'] : '';
        $estado = isset($_POST['estado']) ? $_POST['estado'] : '';
        $descricao = isset($_POST['descricao']) ? $_POST['descricao'] : '';
        $ano_producao = isset($_POST['ano_producao']) ? $_POST['ano_producao'] : '';
        $dimensao = isset($_POST['dimensao']) ? $_POST['dimensao'] : '';
        $idiomas = isset($_POST['idiomas']) ? $_POST['idiomas'] : array();
        
        //Obra Literária:
        $subtitulo = isset($_POST['subtitulo']) ? $_POST['subtitulo'] : '';
        $titulo_alternativo = isset($_POST['titulo_alternativo']) ? $_POST['titulo_alternativo'] : '';
        $ano_producao_fim = isset($_POST['ano_producao_fim']) ? $_POST['ano_producao_fim'] : '';
        $ano_publicacao_inicio = isset($_POST['ano_publicacao_inicio']) ? $_POST['ano_publicacao_inicio'] : '';
        $ano_publicacao_fim = isset($_POST['ano_publicacao_fim']) ? $_POST['ano_publicacao_fim'] : '';
        $seculo_producao = isset($_POST['seculo_producao']) ? $_POST['seculo_producao'] : '';
        $seculo_publicacao = isset($_POST['seculo_publicacao']) ? $_POST['seculo_publicacao'] : '';
        $seculo_encenacao = isset($_POST['seculo_encenacao']) ? $_POST['seculo_encenacao'] : '';
        $ano_encenacao = isset($_POST['ano_encenacao']) ? $_POST['ano_encenacao'] : '';
        $local_encenacao = isset($_POST['local_encenacao']) ? $_POST['local_encenacao'] : '';
        $personagens = isset($_POST['personagens']) ? $_POST['personagens'] : '';
        $palavra_chave = isset($_POST['palavra_chave']) ? $_POST['palavra_chave'] : '';
        $acervo_id = isset($_POST['acervo_id']) ? $_POST['acervo_id'] : '';
        
        //Mídias
        $arquivos = isset($_POST['arquivos']) ? $_POST['arquivos'] : array();
        $arquivos_substituidos = isset($_POST['arquivos_substituidos']) ? $_POST['arquivos_substituidos'] : array();
        $titulos_arquivos = isset($_POST['titulos_arquivos']) ? $_POST['titulos_arquivos'] : array();
        $descricoes_arquivos = isset($_POST['descricoes_arquivos']) ? $_POST['descricoes_arquivos'] : array();
        $fontes_arquivos = isset($_POST['fontes_arquivos']) ? $_POST['fontes_arquivos'] : array();
        
        $controller = ControleObraLiteraria::getInstance();
        if ($action == 'add') {
            exit($controller->add(//Atributos do Documento:
                                  $titulo, $autores, $generos, $categoria, $fontes, $id_material, 
                                  $editoras, $abrangencia, $direitos, $acervo_id, 
                                  $localizacao, $estado, $descricao, $ano_producao, $dimensao, $idiomas,
                                  //Atributos da Obra Literária:
                                  $subtitulo, $titulo_alternativo, $ano_producao_fim, $ano_publicacao_inicio,
                                  $ano_publicacao_fim, $seculo_producao, $seculo_publicacao, $seculo_encenacao, $ano_encenacao,
                                  $local_encenacao, $personagens, $palavra_chave, $acervo_id,
                                  //Atributos das Mídias:
                                  $arquivos, $titulos_arquivos, $descricoes_arquivos, $fontes_arquivos));
        }
        else {
            $id = isset($_REQUEST['id']) ? $_REQUEST['id'] : '';
            exit($controller->update(//Atributos do Documento:
                                     $id, $titulo, $autores, $generos, $categoria, $fontes, $id_material, 
                                     $editoras, $abrangencia, $direitos, $acervo_id,
                                     $localizacao, $estado, $descricao, $ano_producao, $dimensao, $idiomas,
                                     //Atributos da Obra Literária:
                                     $subtitulo, $titulo_alternativo, $ano_producao_fim, $ano_publicacao_inicio,
                                     $ano_publicacao_fim, $seculo_producao, $seculo_publicacao, $seculo_encenacao, $ano_encenacao,
                                     $local_encenacao, $personagens, $palavra_chave, $acervo_id,
                                     //Atributos das Mídias:
                                     $arquivos, $arquivos_substituidos, $titulos_arquivos, $descricoes_arquivos, $fontes_arquivos));
        }
        break;
    
    case 'search_obra':
    	$controller = ControleObraLiteraria::getInstance();
        $term = isset($_REQUEST['term']) ? $_REQUEST['term'] : '';
        $results = $controller->get(null, array('id', 'titulo'), 0, 15, $term);
        $response = array();
        foreach ($results as $key=>$result) {
           $response[$key]['id'] = $result['id'];
           $response[$key]['label'] = $result['titulo'];
           $response[$key]['value'] = $result['titulo'];
        }
        exit(json_encode($response));
        break;

    case 'removeDoc':
    case 'del':
        $ids = isset($_REQUEST['ids']) ? $_REQUEST['ids'] : '';
        $controller = ControleObraLiteraria::getInstance();
        exit($controller->del($ids));
        break;

    case 'upload':
    	$xhr = false;
    	$file = null;
	    if (isset($_GET['qqfile'])){
	        $xhr = true;
	    } elseif (isset($_FILES['qqfile'])){
	        $file = $_FILES['qqfile'];
	    }
        
        $controller = ControleObraLiteraria::getInstance();
        exit($controller->upload($xhr, $file));
        break;
        
    case 'download':
        
        $id = isset($_GET['id']) ? $_GET['id'] : '';
        
        $controller = ControleDocumentos::getInstance();
        $midia = $controller->get_midias(null, array('Documento_id, nome_arquivo'), $id);
        if ($midia) {
        	$midia = $midia[0];
        }
        else {
        	exit(__('Mídia não encontrada'));
        }
        
        //codigo para incrementar a visita da midia
        $controller->visita_midia($id);

        //codigo para adaptabilidade
        if (Auth::check()){
            $id_usuario = isset($_SESSION['id']) ? $_SESSION['id'] : '';
            $id_obra = $midia['Documento_id'];
            $documento = $controller->get($id_obra, array('Genero_id'));
            $documento = current($documento);
            $id_genero = $documento['Genero_id'];
            $id_autores = $controller->get_autores($id_obra,  array('id'));
            $controller_usuario = ControleUsuarios::getInstance();
            $controller_usuario->atualiza_adaptabilidade($id_usuario, $id_autores, $id_genero,$id_obra);
        }
        
        header('Location: ' . DOCUMENTS_URI . $midia['nome_arquivo']);
        
        break;
        
    case 'getTableData':
        $controller = ControleObraLiteraria::getInstance();
        $_GET['sTable'] = 'ObraLiteraria'; //Nome da tabela
        $_GET['iTypeId'] = DOCUMENTOS_OBRA_LITERARIA_ID; //ID do tipo do documento
        exit($controller->getTableData($_GET));
        break;
        
    default:
        require_once(APPLICATION_PATH . '/include/TemplateHandler.php');
        $template = new TemplateHandler();
        $template->set_css_files(array(
                                'admin.css',
                                'jquery.dataTables.css'
                                ));
        $template->set_js_files(array(
                                'jquery.dataTables.min.js',
                                'jquery.loadTable.js'
                                ));
        $template->set_content_file('admin/documentos/obra-literaria/obra-literaria.php');
        $template->set_authenticated_only(true);
        $template->set_authentication_list(array(PAPEL_ADMINISTRADOR_ID, PAPEL_CADASTRADOR_ID));
        $template->set_header_file('header_admin.php');
        $template->set_body_file('body_admin.php');
        $template->set_navigation_file('navigation_admin.php');
        $template->set_active_nav_item(ADMIN_DOCUMENTOS_ID);
        $template->show();
}
