...
switch ($action) {
	...
	case 'download' :
		...
		if ($midia) {
			...
			echo __('Arquivo nao encontrado, procure novamente');
			echo '"))'."\n" ;
			echo	'window.location="./../busca/index.php#documento";';
		...
		if (isset($config['recommendation']) AND $config['recommendation']) {
			if (Auth::check ()) {
				$id_usuario = isset ( $_SESSION ['id'] ) ? $_SESSION ['id'] : '';
				$id_obra = $midia ['Documento_id'];
				
				if ($config['recommendation_type']==1) {
					$documento = $controller->get ( $id_obra);
					$documento = current ( $documento );
					$generos = $controller->get_generos_new($documento['Documento_id']);
					...
					$controller_recomendacao = ControleRecomendacao::getInstance ();
					$controller_recomendacao->registra_acesso($id_usuario, $id_obra );
				} 
			}
		}
		/*
			...
			if (Auth::check ()) {
				...
				if ($usuario_logado ['anotacao']) {
					...
					*/
					...
					exit ( file_get_contents ('/index.php') );
				}
			}
