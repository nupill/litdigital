...
require_once(APPLICATION_PATH . '/controllers/ControleFontes.php');
$id = isset($_GET['id']) ? $_GET['id'] : '';
if (!$id) {
    exit(__('ID não especificado'));
}
$controle_fontes = ControleFontes::getInstance();
$fonte = $controle_fontes->get($id);
if (!$fonte) {
...
<div id="content">
	<h2>
		<a href="<?php echo ADMIN_URI; ?>"><?php echo __('Área administrativa');?></a> &raquo;
	 	<a href="<?php echo ADMIN_FONTES_URI; ?>"><?php echo __('Fontes');?></a> &raquo;
	 	<?php echo __('Editar fonte');?>
	</h2>
	<br />
	<br />
    <form id="update" action="<?php echo ADMIN_FONTES_URI; ?>?action=update&id=<?php echo $id; ?>" method="post" class="inline">
        <fieldset>
            <legend><?php echo __('Utilize o campo abaixo para editar a fonte bibliográfica');?></legend>
            <p><label for="descricao"><?php echo __('Descrição');?>*</label>
            <input type="text" id="descricao" name="descricao" style="width: 900px" value="<?php echo $fonte['descricao']; ?>" /></p>
        </fieldset>
    ...
    var options = {
        ...
        success: function(response, status) {
            ...
            if (response && response.error == null) {
                $('#status').html('<span class="success">'+"<?php echo __('Fonte atualizada com sucesso');?>"+'!</span>');
            }
            else {
